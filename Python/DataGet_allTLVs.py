...
#Number of rows and columns TLV type 5
NUMBER_OF_ROWS = 63
...
def RangeDopplerHM (byteBuffer):
        ...
        # Initialize the pointer index at numTLVs location
        idX = 32
        
        # Read the header to extract the number of TLVs
        ...
        for tlvIdx in range(numTLVs):
            ...
            if tlv_type == 5:
                ...
                resultSize = NUMBER_OF_COLUMNS * (NUMBER_OF_ROWS + 1) * np.dtype(np.uint16).itemsize    # a revoir pour adapter au nv config file
                ...
                idX+=tlv_length
        #If there is no TLV type 5 in the packet
        return(dataOK,mat)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            dataOk, matm, Objectsdata = update()                    # TROUVER OU STOCKER OBJECTSDATA
            if dataOk > 0:
                if count != 0:
            ...
            time.sleep(0.5) # Sampling frequency of 2 Hz
