...
configFileName = os.getcwd() + '\\cross_feature' + '\\config_file_doppler_azimuth_32x256.cfg'

# CNN 3d model, min and max values
model_name = os.getcwd() + '\\cross_feature' + '\\all_targets_cross_1080.h5'
    ...
    for tlvIdx in range(numTLVs):
        ...
        if tlv_type == 4:
            ...
            if tlv_length == expected_size:
                ...
                inpt_az = abs(Q)                                                      # Magnitude of the fft
                np.fliplr(inpt_az)
                if inpt_az.shape == (RANGE_FFT_SIZE,DOPPLER_FFT_SIZE):
                    dataOK = 1
                    if DEBUG == True:
        ...
        if tlv_type == 5:
            ...
            if tlv_length == resultSize:
                ...
                mat = result[1:, :]
                inpt_dop = mat
                if inpt_dop.shape == (DOPPLER_FFT_SIZE, RANGE_FFT_SIZE):
                    dataOK = 1
                    if DEBUG == True:
...
def update():
    ...
    cat = np.zeros((inpt_az.shape[0],inpt_az.shape[1]*2),dtype=np.float32)
    if dataOk > 0:
        # Calculate the probability of classes for 3D CNN
        inpt_az = np.transpose(inpt_az)
        inpt_az = norm(inpt_az,'Azimuth')
        inpt_dop = norm(inpt_dop,'Doppler')
        cat = np.concatenate((inpt_az,inpt_dop))
        cat = cat.reshape(1,
                        RANGE_FFT_SIZE,
                        DOPPLER_FFT_SIZE*2,
                        1)
        rt = model.predict(cat, verbose=0)[0]
        print("Class probabilities:",rt)
        if rt[0] > 1-THRESHOLD:
            clas = label[0]
        elif rt[0] < 1-THRESHOLD and rt[1] > 1-THRESHOLD:
            clas = label[1]
        elif rt[0] < 1-THRESHOLD and rt[2] > 1-THRESHOLD:
            clas = label[2]
        print("Class :",clas,"\n")
    return dataOk, clas
...
inpt_dop = np.zeros((DOPPLER_FFT_SIZE, RANGE_FFT_SIZE), dtype = np.float32)   # Input sample for the CNN

inpt_az = np.zeros((RANGE_FFT_SIZE, DOPPLER_FFT_SIZE), dtype = np.float32)   # Input sample for the CNN
