...
model_name_dop = os.getcwd() + '\\2sensors\\Features\\all_targets_doppler_1211_6587.h5'
min_dop = 1211.0
max_dop = 6587.0

# CNN 2D model, min and max values
model_name_az = os.getcwd() + '\\2sensors\\Features\\all_targets_azimuth_0_37230.h5'
min_az = 0
max_az = 37230
...
def update():
    ...
    global inpt_dop, inpt_az, compteur
    dataOk = 0
    clas = 'classe'
    pred_dop = []
    pred_az  = []
    ...
    compteur += 1
    PacketBuffer = readData(Dataport1)
    dataOk1 = parseData68xx(PacketBuffer)
    inpt_dop1 = inpt_dop
    inpt_az1 = inpt_az
    PacketBuffer = readData(Dataport2)
    dataOk2 = parseData68xx(PacketBuffer)
    inpt_dop2 = inpt_dop
    inpt_az2 = inpt_az
    dataOk = dataOk1 + dataOk2
    inpt_az = inpt_az1 + inpt_az2
    inpt_dop = inpt_dop1 + inpt_dop2
    if dataOk > 1:
        ...
        return dataOk, 'Error', np.zeros((RANGE_FFT_SIZE,DOPPLER_FFT_SIZE),dtype=np.float32), np.zeros((DOPPLER_FFT_SIZE,RANGE_FFT_SIZE),dtype=np.float32)
